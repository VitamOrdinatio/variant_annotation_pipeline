...
Purpose:
    Measure whether the ERR10619300 WES epilepsy VAP-TEP faithfully transports
...
TEP_NAME = "vap_tep_ERR10619300_run_2026_05_27_172531_v1"
...
EXPECTED = {
    # ERR10619300 is a WES epilepsy SRA. Exact counts are measured from the
...
def ensure_output_dir(repo_root: Path) -> Path:
    ...
    out_dir = DESKTOP_DIR / f"err10619300_vap_tep_certification_audit_{timestamp}"
...
def audit_transport_fidelity(out_dir: Path) -> List[Dict[str, Any]]:
    ...
    for role, stage, source, tep, kind in ARTIFACT_PAIRS:
        ...
        rows.append(row)
    write_tsv(out_dir / "tep_err10619300_entity_row_count_audit.tsv", rows, ["role", "stage", "kind", "source_path", "tep_path", "source_exists", "tep_exists", "source_size_bytes", "tep_size_bytes", "source_sha256", "tep_sha256", "sha256_match", "source_row_count", "tep_row_count", "row_count_match", "source_column_count", "tep_column_count", "column_count_match"])
    return rows
...
def audit_required_columns(out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_tsv(out_dir / "tep_err10619300_required_column_audit.tsv", rows, ["entity", "path", "column_check_type", "column_or_group", "present", "matched_column"])
    return rows
...
def audit_variant_parity(out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_tsv(out_dir / "tep_err10619300_variant_id_parity_audit.tsv", rows, ["comparison", "left_entity", "right_entity", "left_variant_id_count", "right_variant_id_count", "intersection_count", "left_only_count", "right_only_count", "sets_equal", "row_count_match", "column_count_match", "sha256_match", "semantic_note"])
    return rows
...
def audit_stage08_routing_overlap(out_dir: Path) -> Dict[str, Any]:
    ...
    write_tsv(out_dir / "tep_err10619300_stage08_routing_overlap_audit.tsv", [{"metric": k, "value": v} for k, v in summary.items()], ["metric", "value"])
    return summary
...
def audit_candidate_collapse(out_dir: Path) -> Tuple[List[Dict[str, Any]], Dict[str, Any], Dict[str, Any]]:
    ...
    write_tsv(out_dir / "tep_err10619300_candidate_collapse_audit.tsv", rows, ["section", "metric", "value", "expected", "status"])
...
def audit_lineage_integrity(out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_tsv(out_dir / "tep_err10619300_lineage_integrity_audit.tsv", rows, ["check", "status", "value", "details"])
    return rows
...
def audit_stage13_context(out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_tsv(out_dir / "tep_err10619300_stage13_context_audit.tsv", rows, ["artifact", "check", "source_exists", "tep_exists", "source_sha256", "tep_sha256", "status", "details"])
    return rows
...
def audit_trace_examples(out_dir: Path) -> None:
    ...
    write_tsv(out_dir / "tep_err10619300_random_variant_trace_examples.tsv", random_rows, fieldnames)
    write_tsv(out_dir / "tep_err10619300_edge_case_variant_trace_examples.tsv", edge_rows, fieldnames)
...
def write_certification_summary_md(out_dir: Path, scientific_summary: Dict[str, Any]) -> None:
    ...
    lines = [
        "# ERR10619300 VAP-TEP Certification Probe Summary", "",
        "## Scope", "",
        f"TEP: `{TEP_NAME}`", "",
        f"Run: `{RUN_ID}`", "",
        f"Sample: `{SAMPLE_ID}`", "",
    ...
    lines.extend(["", "## Stage08 Routing Model", "", "Expected interpretation:", "", "```text", "Coding Partition", "Noncoding Partition", "Splice Overlay", "```", "", "Routing overlap summary is available in:", "", "```text", "tep_err10619300_stage08_routing_overlap_audit.tsv", "```", "", "## Important Semantic Note", "", "Physical equivalence of Stage08 selected-transcript and VDB-ready artifacts in VAP v1", "does not imply semantic equivalence forever. Both artifact identities must remain preserved.", "", "## Output Files", "", "See all TSV and JSON audit outputs in this directory.", ""])
...
def main() -> int:
    ...
    try:
        ...
        (out_dir / "tep_err10619300_trace_examples_error.txt").write_text(repr(exc), encoding="utf-8")
