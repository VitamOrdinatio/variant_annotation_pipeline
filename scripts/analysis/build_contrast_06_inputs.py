...
CONTRAST_DIR = Path("docs/case_studies/cross_runs/contrasts/contrast_06_inputs")
MANIFEST_PATH = CONTRAST_DIR / "cohort_manifest.tsv"
AUDIT_PATH = CONTRAST_DIR / "contrast_06_input_build_audit.tsv"
...
def maybe_create_readme() -> None:
    ...
    text = """# Contrast 06 Inputs — Candidate Reviewability Stability

This directory contains the governed input substrate for Contrast 06.

The bundle is manifest-driven, SRA-partitioned, and contains compact reviewability summaries, priority-tier summaries, substrate dimension summaries, targeted semantic bucket outputs, and unique-gene outputs.
...
def main() -> None:
    ...
    for _, rec in manifest.iterrows():
        ...
        unique_genes_dir = sra_dir / "sql_outputs" / "unique_genes"

        for d in [tables_dir, buckets_dir, unique_genes_dir]:
        ...
        # Unique-gene outputs
        copy_all_tsvs(
            source_dir=run_root / "logs" / "stage12_exploration" / "unique_genes",
            dest_dir=unique_genes_dir,
            sra=sra,
            run_id=run_id,
            depth_category=depth_category,
            artifact_group="unique_genes",
            audit=audit,
            notes="copied without renaming or content modification",
        )
