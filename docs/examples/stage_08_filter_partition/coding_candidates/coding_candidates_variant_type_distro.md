...
```bash
cut -f $(head -n1 coding_candidates.tsv | tr '\t' '\n' | nl | grep variant_type | awk '{print $1}') coding_candidates.tsv | sort | uniq -c > docs/examples/stage_08_filter_partition/coding_candidates/coding_candidates_variant_type_distro.md
```
